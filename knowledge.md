...
| continued .... | If the CORE_CHAINCODE_STARTUPTIMEOUT method does not work, the best way is to add a NPM intermediate caching scheme, and a specific step can look at the https://github.com/hyperledger/composer/issues/3974 ;NPM dependencies are downloaded for the first time, so REQUEST_TIMEOUT will appear at the first time. However, when the NPM cache is complete, you use caching when you execute the second time.;Repeat OK several times and pay attention to the persistent directory /home/repo. of container npmregistry.;The core of this process is to use the NPM cache to change the hyperledger-ccenv compilation time from ten minutes to three minutes; in this process, pay attention to the related cache container log:Docker logs -f npmregistry
